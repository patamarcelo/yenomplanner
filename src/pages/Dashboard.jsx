...
function StackedDayTooltip({ active, payload, label, catMeta, money, theme }) {
  if (!active || !payload || payload.length === 0) return null;

  // payload inclui entradas para cada categoria + "total"
  const items = (payload || [])
    ...
    >
      <Stack spacing={0.8}>
        ...
        {items.slice(0, 10).map((it) => {
          ...
          return (
            <Stack key={String(it.dataKey)} direction="row" spacing={1} alignItems="center" justifyContent="space-between">
...
export default function Dashboard() {
  ...
  const resolveAccountIdFromTxn = useMemo(() => {
    return (t) => {
      // backend: account_id / account / account.id / ui: accountId
      const apiAcc =
        t?.accountId ??
        t?.account_id ??
        t?.account?.id ??
  ...
  }, [accountsById, resolveAccountIdFromTxn]);
  ...
  const getTxnMonthKey = useMemo(() => {
    return (t) => {
      // ✅ Cartão: se existir invoiceMonth, consideramos "mês da fatura"
      const invYM = normalizeYM(t?.invoiceMonth ?? t?.invoice_month ?? "");
      if (invYM) return { kind: "card", ym: invYM };

      // ✅ Não-cartão: mês pela compra (fallback chargeDate)
  ...
  const monthTx = useMemo(() => {
    ...
    // ✅ regra nova: cartão usa sempre a fatura do mês seguinte
    ...
    const out = (txns || []).filter((t) => {
      ...
      if (k.kind === "card") {
        return k.ym === targetInvoiceYM; // ex: selecionou fev -> fatura mar
      }

      return k.ym === month; // não-cartão segue o mês selecionado
    });
  ...
  const totalCartoesMes = useMemo(() => {
    return sum(
      monthTx
        .filter((t) => normalizeYM(t?.invoiceMonth ?? t?.invoice_month ?? "")) // tem invoiceMonth => cartão
  ...
  // ✅ Despesas mensais = total das BILLS no mês selecionado
  // ✅ Despesas mensais = soma das BILLS recorrentes ativas no mês (startMonth..endMonth)
  ...
  const monthConsumptionTx = useMemo(() => {
    if (!month) return [];
    ...
    return (txns || []).filter((t) => {
      ...
      // ✅ consumo: mês selecionado pela data de compra
      if (ymFromISODate(pd) !== month) return false;

      // ✅ só despesas no gráfico
      return normalizeDirectionFromTxn(t) === "expense";
    });
  }, [txns, month]);
  ...
  const chartStack = useMemo(() => {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      byDay.set(String(d), { label: String(d), total: 0 });
    }

    const catMeta = new Map();   // key -> {name, color}
    const catKeysSet = new Set();

    for (const t of monthConsumptionTx || []) {
      const pd = normalizeISODate(t?.purchaseDate ?? t?.purchase_date);
      const day = String(dayFromISODate(pd) || "");
      if (!byDay.has(day)) continue;

      const catId = String(t?.categoryId ?? t?.category_id ?? t?.category ?? "outros");
      const cat = categoriesById?.get?.(catId); // se você estiver usando Map
      const key = cat ? String(cat.id) : catId;
      ...
      const v = Math.abs(Math.min(0, signedAmountNormalized(t))); // despesa positiva
    ...
    // ordenar categorias por total (maior primeiro)
  ...
  }, [month, monthConsumptionTx, categoriesById]);
  ...
  const categoriesRank = useMemo(() => {
    const rowsMap = new Map(); // key -> { key, name, color, total }

    for (const t of monthConsumptionTx || []) {
      // monthConsumptionTx já é expense + purchaseDate no month
      const catId = String(t?.categoryId ?? t?.category_id ?? t?.category ?? "outros");
      ...
      cur.total += v;
      // se antes não tinha cor e agora tem, atualiza
  ...
  }, [monthConsumptionTx, categoriesById]);



  const invoicesByCard = useMemo(() => {
    const sums = new Map(); // accId -> cents/amount (em reais aqui)

    for (const t of monthTx || []) {
      // ✅ cartão => tem invoiceMonth (mês da fatura)
      const invYM = normalizeYM(t?.invoiceMonth ?? t?.invoice_month ?? "");
      if (!invYM) continue;
      ...
      sums.set(accId, (sums.get(accId) || 0) + v);
    }
    ...
    const out = Array.from(sums.entries())
      .map(([accId, total]) => {
        ...
        return {
          ...
          // opcional: se você ainda não quer mexer com due date agora, mantém simples:
          dueLabel: "—",
        };
      })
      .sort((a, b) => (b.total || 0) - (a.total || 0));
    ...
    return out;
  }, [monthTx, accountsById, resolveAccountIdFromTxn]);
  ...
  return (
    <Stack spacing={2.25} sx={{ width: "100%" }}>
      ...
      <Box sx={{ width: "100%", display: "grid", gap: 2, alignItems: "stretch", gridTemplateColumns: { xs: "1fr" } }}>
        <Card sx={(t) => cardBg(t, "primary")}>
          <CardContent sx={{ p: 2.25 }}>
            <Stack spacing={1.2}>
              <Stack direction="row" justifyContent="space-between" alignItems="baseline">
                <Typography sx={{ fontWeight: 950, letterSpacing: -0.4 }}>Despesas por dia de consumo</Typography>
                <Typography variant="caption" sx={{ color: "text.secondary" }}>referência (data de compra) • {formatMonthBR(month)}</Typography>
              </Stack>
              <Box sx={{ width: "100%", height: 360, minHeight: 360 }}>
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={chartStack.data} margin={{ top: 26, right: 18, left: 0, bottom: 0 }}>
                    <CartesianGrid vertical={false} stroke={alpha(theme.palette.divider, 0.55)} strokeDasharray="3 6" opacity={0.18} />
                    <XAxis dataKey="label" tick={{ fontSize: 12 }} axisLine={false} tickLine={false} />
                    <YAxis tick={{ fontSize: 12 }} axisLine={false} tickLine={false} width={60} domain={[0, Math.max(10, maxChartValue * 1.15)]} tickFormatter={(v) => (hideValues ? "••••" : formatBRL(v))} />
                    <RechartsTooltip content={(props) => <StackedDayTooltip {...props} catMeta={chartStack.catMeta} theme={theme} money={money} />} />
                    {chartStack.catKeys.map((k) => {
                      const meta = chartStack.catMeta.get(String(k));
                      const fill = meta?.color || pickFallbackColor(theme, k);
                      return <Bar key={k} dataKey={k} stackId="day" fill={fill} radius={[0, 0, 0, 0]} barSize={24} isAnimationActive animationDuration={500} />;
                    })}
                    <Bar dataKey="total" fill="transparent" stackId="__total_label__">
                      <LabelList dataKey="total" position="top" content={({ x, y, width, value }) => {
                        if (!value || Number(value) <= 0) return null;
                        return <text x={x + width / 2} y={y - 6} textAnchor="middle" fontSize={9} fontWeight={700} fill={theme.palette.text.secondary}>{money(value)}</text>;
                      }} />
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
      ...
      {/* Categorias + Faturas */}
      <Box sx={{ width: "100%", display: "grid", gap: 2, minHeight: "350px", alignItems: "stretch", gridTemplateColumns: { xs: "1fr", md: "1fr 1fr" } }}>
            ...
            <Stack spacing={1} sx={{ height: "100%" }}>
              <Stack direction="row" justifyContent="space-between" alignItems="baseline">
                <Stack spacing={0.2}>
                  ...
                  <Typography variant="caption" sx={{ color: "text.secondary" }}>referência financeira • {formatMonthBR(month)}</Typography>
                </Stack>
                      ...
                      return (categoriesRank.rows || []).map((row, idx) => {
                        ...
                        return (
                          <React.Fragment key={row.key}>
                            <Box sx={(t) => ({ py: 1, display: "flex", alignItems: "center", gap: 1, borderRadius: 1.5, px: 0.5, transition: "background 140ms ease", "&:hover": { background: alpha(t.palette.action.hover, 0.7) } })}>
                              ...
                              <Box sx={{ minWidth: 0, flex: 1 }}>
                                ...
                                <Box sx={{ mt: 0.55 }}>
                                  <LinearProgress variant="determinate" value={pct} sx={{ height: 4, borderRadius: 999, background: alpha(base, 0.12), "& .MuiLinearProgress-bar": { borderRadius: 999, backgroundColor: alpha(base, 0.95) } }} />
                                </Box>
                              </Box>
                              <Typography sx={{ fontWeight: 950, fontSize: 13, whiteSpace: "nowrap" }}>{money(row.total)}</Typography>
                            </Box>
            ...
            <Stack spacing={1} sx={{ height: "100%" }}>
              <Stack spacing={0.4}>
                <Stack direction="row" justifyContent="space-between" alignItems="baseline">
                  ...
                  <Typography sx={{ fontWeight: 950, fontSize: 15, letterSpacing: -0.2 }}>{money(totalInvoicesMonth)}</Typography>
                </Stack>
                <Stack direction="row" justifyContent="space-between" alignItems="center">
                  <Typography variant="caption" sx={{ color: "text.secondary" }}>
                    baseado no invoiceMonth • {formatMonthBR(addMonthsYM(month, 1))}
                  </Typography>
                  <Typography variant="caption" sx={{ color: "text.secondary", fontWeight: 800 }}>{(invoicesByCard || []).length} cartões</Typography>
                </Stack>
              </Stack>
              <Divider />
              <Box sx={{ flex: 1, overflow: "auto" }}>
                      ...
                      return (invoicesByCard || []).map((c, idx) => {
                        ...
                        return (
                          <React.Fragment key={c.id}>
                            <Box sx={(t) => ({ py: 1, display: "flex", alignItems: "center", gap: 1, borderRadius: 1.5, px: 0.5, transition: "background 140ms ease", "&:hover": { background: alpha(t.palette.action.hover, 0.7) } })}>
                              <Box sx={{ width: 10, height: 10, borderRadius: 999, background: base, flexShrink: 0, boxShadow: `0 0 0 3px ${alpha(base, 0.12)}` }} />
                              <Box sx={{ minWidth: 0, flex: 1 }}>
                                <Typography sx={{ fontWeight: 950, fontSize: 13 }} noWrap>{c.name}</Typography>
                                <Typography sx={{ mt: 0.15, fontSize: 9.6, fontWeight: 850, color: "text.secondary", lineHeight: 1.1 }} noWrap>Vence aprox. {c.dueLabel}</Typography>
                                <Box sx={{ mt: 0.75 }}>
                                  <LinearProgress variant="determinate" value={pct} sx={{ height: 4, borderRadius: 999, background: alpha(base, 0.14), "& .MuiLinearProgress-bar": { borderRadius: 999, backgroundColor: alpha(base, 0.92) } }} />
                                </Box>
                              </Box>
                              <Typography sx={{ fontWeight: 950, fontSize: 13, whiteSpace: "nowrap" }}>{money(c.total)}</Typography>
                            </Box>
